...
/**
 @brief A method to return current implementation of platform implementation callbacks for FIPS compliance.

 @param callbacks A location(address) to store the Callbacks struct. Please dealloc/free any previous information, to avoid any memory leaks.

 @return BOOL indicating whether the callbacks were successfully extracted into the location or not.

 */
BOOL extractCurrentPlatformCallbacks(void **callbacks);
