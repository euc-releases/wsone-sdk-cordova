...
//  Airwatch Plugin
//
//  Copyright (C) 2016 - 2020 VMWare AirWatch. All rights reserved.
...
import com.airwatch.notification.*;
...
public class AirwatchBridge {
    ...
    public boolean allowCopyPaste() throws AirWatchSDKException {
       SDKConfiguration config=SDKContextManager.getSDKContext().getSDKConfiguration();
        String allowCopyPaste = config.getValuesWithKeyStartWith(SDKConfigurationKeys.TYPE_DATA_LOSS_PREVENTION,SDKConfigurationKeys.ENABLE_COPY_PASTE);
        if(TextUtils.isEmpty(allowCopyPaste)){
           return true;
        }
        return Boolean.valueOf(allowCopyPaste);
    }
    ...
    public void registerPushNotificationToken(String token) throws AirWatchSDKException
    {
        PushNotificationManager.getInstance(cordovaActivity.getApplicationContext()).registerToken(token);
    }
    public void processMessage(String title, String body,Map<String, Object> payload )
    {
        PushNotificationManager.getInstance(cordovaActivity.getApplicationContext()).processMessage(title, body, payload);
    }
}
